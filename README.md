...
| **analysis**               | Top-level reference-size keys: `rmsd_max_train`, `rmsd_max_test`, `q_max_train`, `q_max_test`, `coord_clustering_max_train`, `coord_clustering_max_test`, `distmap_clustering_max_train`, `distmap_clustering_max_test` (null = use all; int = cap reference set for that metric). Nested blocks: `rmsd_gen`, `rmsd_recon`, `q_gen`, `q_recon`, `coord_clustering_gen`, `coord_clustering_recon`, `distmap_clustering_gen`, `distmap_clustering_recon`. Recon blocks use `max_recon_train` / `max_recon_test` for how many structures to **reconstruct**; the shared max_* keys cap the **reference** set used for comparisons. Analysis (RMSD, Q, both clusterings) runs only on Euclideanizer outputs. |
...
| **analysis.coord_clustering_recon**                                                               | Nested block: `enabled`, `overwrite_existing`, `max_recon_train`, `max_recon_test`, `n_subsample`, `save_data`, `save_pdf_copy`, `visualize_latent`. If enabled, coord clustering (recon) figures under `analysis/coord_clustering/recon/`.                                                                                                                                              |
...
| **analysis.distmap_clustering_recon**                                                            | Nested block: `enabled`, `overwrite_existing`, `max_recon_train`, `max_recon_test`, `n_subsample`, `save_data`, `save_pdf_copy`, `visualize_latent`. If enabled, distmap clustering (recon) figures under `analysis/distmap_clustering/recon/` and optional `latent_distribution.png`, `latent_correlation.png` when `visualize_latent`.                                                     |
...
- **Euclideanizer run**: `output_dir/seed_<n>/distmap/<i>/euclideanizer/<j>/`
  ...
  - When RMSD (gen) is enabled: `analysis/rmsd/gen/<run_name>/` per (num_samples, variance), with `rmsd_distributions.png`, optional `data/`, optional `structures/`. When RMSD recon is enabled: `analysis/rmsd/recon/` with `rmsd_distributions.png` and optional `latent_distribution.png`, `latent_correlation.png` (if `visualize_latent`). When Q (gen) is enabled: `analysis/q/gen/<run_name>/` with `q_distributions.png` and optional `data/`, `structures/`. When Q (recon) is enabled: `analysis/q/recon/` (or `.../recon/<subdir>/` for multiple sizes) with `q_distributions.png` and optional `latent_distribution.png`, `latent_correlation.png` (if `visualize_latent`). When Coord clustering (gen) is enabled: `analysis/coord_clustering/gen/<run_name>/` with `pure_dendrograms.png`, `mixed_dendrograms.png`, `mixing_analysis.png`, `rmse_similarity.png`. When Coord clustering (recon) is enabled: `analysis/coord_clustering/recon/` (or `.../recon/<subdir>/`) with the same four figures. When Distmap clustering (gen) is enabled: `analysis/distmap_clustering/gen/<run_name>/` with the same four figures. When Distmap clustering (recon) is enabled: `analysis/distmap_clustering/recon/` (or `.../recon/<subdir>/`) with the same four figures and optional `latent_distribution.png`, `latent_correlation.png` (if `visualize_latent`).
...
- **Recon:** Reconstruct train and test structures; compute one-to-one RMSD (reconstructed vs original). One figure with test→train (reused from cache), train recon RMSD, test recon RMSD. Outputs: `analysis/rmsd/recon/rmsd_distributions.png`, optional `latent_distribution.png` and `latent_correlation.png` when `visualize_latent` is true. Config: `analysis.rmsd_recon` (`max_recon_train`, `max_recon_test`, `save_data`, `save_pdf_copy`, `visualize_latent`).
...
- **Recon:** One figure with test→train max Q, train recon Q (one-to-one), test recon Q (one-to-one). Outputs under `analysis/q/recon/` (or `.../recon/<subdir>/` for multiple sizes). Optional latent figures when `visualize_latent`. Config: `analysis.q_recon`.
...
- **Four figures** (gen: `analysis/coord_clustering/gen/<run_name>/` or `analysis/distmap_clustering/gen/<run_name>/`; recon: `analysis/coord_clustering/recon/` or `analysis/distmap_clustering/recon/` or `.../recon/<subdir>/` for each):
  ...
  - **mixing_analysis.png** — Bar chart of mixing scores (observed vs expected) and cluster composition. The composition panels use an **IQR-based gap cut**: merge heights are sorted, gaps between consecutive heights are computed, and a gap is treated as an outlier if it exceeds Q3 + 1.5×IQR (same rule as a boxplot). The tree is cut at the midpoint of the largest such outlier gap (or the largest gap if none exceed the bound), so the number of clusters is derived from the tree with no manual parameter. This avoids one giant cluster plus many singleton outlier clusters and makes source fractions interpretable. Each cluster bar is labeled **n=** with the number of structures in that cluster.
...
### Latent distribution and correlation

When **visualize_latent** is true for an analysis recon block (RMSD, Q, or Clustering), the pipeline also produces:

- **latent_distribution.png** — Box plots (train vs test) and mean/std per latent dimension for the reconstructed structures.
- **latent_correlation.png** — Two panels: train vs test **mean** and train vs test **std** across dimensions, with dashed y=x and Pearson r and R². Measures how well the latent statistics of train and test align.
