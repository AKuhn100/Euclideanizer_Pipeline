...
When **`scoring.enabled`** is true, the pipeline runs **scoring after each block** (after plotting, after latent, and after each analysis metric—RMSD, Q, coord clustering, distmap clustering). Each run’s **`scoring/`** directory is updated with **`scores.json`** and a **spider/radar plot** (`scores_spider.png`; optional PDF when **`scoring.save_pdf_copy`** is true). Scores are computed from NPZ produced by plotting and analysis; the scheme is in `Pipeline/SCORING_SPEC.md` (z-score normalization, MAE/EMD, geometric mean). The dashboard copies the spider image from `scoring/` into `dashboard/assets/` and shows it (and overall score) in the Detail view.
