...
| **scoring**                | `enabled`, `overwrite_existing`, **`tau_config`** (path to YAML: one τ > 0 per scoring component; see `samples/scoring_tau_sample.yaml`), `save_pdf_copy`. When enabled, per-run scores and spider plot are written under `scoring/`; see **Scoring** subsection.                                                                                                                                                                                                                                                                                                                                                                                                              |


### Scoring

When `**scoring.enabled`** is true, the pipeline runs **scoring after each block** (after plotting, after latent, and after each analysis metric—RMSD, Q, coord clustering, distmap clustering, generative capacity when enabled). Each run’s `**scoring/`** directory is updated with `**scores.json**` and a **spider/radar plot** (`scores_spider.png`; optional PDF when `**scoring.save_pdf_copy`** is true). **`scoring.tau_config`** must point to a YAML file listing every scoring component and its τ for `score = exp(−d/τ)` (required; no default in code—use `samples/scoring_tau_sample.yaml` for τ = 1 everywhere). Scores are computed from NPZ produced by plotting and analysis; the scheme is defined in the scoring specification shipped with the pipeline (z-score normalization, MAE/Wasserstein, geometric mean). The dashboard copies the spider image from `scoring/` into `dashboard/assets/` and shows it (and overall score) in the Detail view.
...
**Precompute logging (multi-GPU):** While that main-process phase runs (before any worker subprocesses), the pipeline logs progress: each run directory’s plot split cache (reuse vs slice from global experimental distance maps vs full `compute_exp_statistics`), saves, then per analysis metric which cache files are built or skipped—so large grids do not look hung at “Detected N GPU(s)…”.

**Single-GPU logging:** The same plot split-cache messages (with `[i/n]` across run entries) appear at the start of each `_run_one_seed` call; seed-level analysis caches log when they are first built during analysis (`Analysis seed cache [metric] …`), matching the information multi-GPU precompute prints upfront.
...
- **scoring**: `enabled`, `overwrite_existing`, **`tau_config`** (YAML path for per-component τ), `save_pdf_copy` (when true, save a PDF of the scores spider plot in dashboard assets).
