...
| **analysis**               | Top-level reference-size keys: `rmsd_max_train`, `rmsd_max_test`, `q_max_train`, `q_max_test`, `coord_clustering_max_*`, `distmap_clustering_max_*`, `**latent_max_train`**, `**latent_max_test**` (null = use all; int = cap reference set). Nested blocks: `rmsd_gen`, `rmsd_recon`, `q_gen`, `q_recon`, `coord_clustering_*`, `distmap_clustering_*`, and `**latent**` (standalone; one plot set per Euclideanizer run under `analysis/latent/`; uses latent_max_train / latent_max_test). Recon blocks use `max_recon_train` / `max_recon_test` for how many structures to **reconstruct**. Analysis (RMSD, Q, clusterings, latent) runs only on Euclideanizer outputs. |
...
| **Pairwise distance by lag** | When `plotting.bond_length_by_genomic_distance: true`: three 5×4 grids — (1) train experimental vs recon train, (2) test experimental vs recon test, (3) train, test, and gen overlay. Up to 20 evenly spaced lags k per figure. |
