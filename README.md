...
| **analysis.*.max_recon_train / max_recon_test**                | In `rmsd_recon`, `q_recon`, `clustering_recon`: how many structures to **reconstruct**; independent of the shared reference-size caps above.                                                                                      |
...
Euclideanizer_Pipeline/
  ...
  src/
    ...
    clustering.py        # Clustering / dendrogram analysis (optional, via analysis.clustering_gen / clustering_recon)
...
| **Q (gen)** (analysis)             | When `analysis.q_gen.enabled: true`: histograms of max Q (test→train, gen→train, gen→test) per (DistMap, Euclideanizer) pair. Outputs under `analysis/q/gen/<run_name>/` (figure, optional `data/`, optional `structures/`). Use `analysis.q_max_train`, `q_max_test` for reference size; `num_samples`, `sample_variance`, `delta`, `query_batch_size`, `save_data`, `save_structures_gro`. |
