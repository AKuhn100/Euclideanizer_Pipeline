...
- **Four figures** (gen: `analysis/clustering/gen/<run_name>/`; recon: `analysis/clustering/recon/` or `.../recon/<subdir>/`):
  - **pure_dendrograms.png** — One tree per population (e.g. Training only, Generated only, Test only). Each leaf is one subsampled structure; height is distance at which clusters are merged (UPGMA / average linkage). Shows how spread out each population is internally.
  ...
  - **rmse_similarity.png** — Q–Q plots comparing **within-population** pairwise RMSE distributions (e.g. Training vs Generated). Each point is (quantile value in A, quantile value in B). Points on the diagonal mean the two populations have similar internal distance structure; Pearson r in the title summarizes that match.
