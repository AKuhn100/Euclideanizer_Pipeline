...
- **Setup wizard entrypoint**: `run_setup_wizard.py`. Guides from raw data (file or directory) to a converted NPZ file using the Claude API to generate a custom converter script. Requires `ANTHROPIC_API_KEY`. Generated scripts go to `setup_wizard_scripts/`. See `specs/SETUP_WIZARD.md`.
...
- **Dashboard scan matches run layout.** `src/dashboard._scan_runs()` treats each top-level `seed_<n>/` or `seed_<n>_split_<frac>/` (multiple `data.training_split` values) as a seed group; run IDs are `seed_<n>_dm_*` / `_eu_*` or `seed_<n>_split_<frac>_dm_*` / `_eu_*` so Browse/Compare stay unique per (split_seed, training_split) pair.
...
- **Generative capacity figure:** **`GEN_CAP_STACKED_FIGWIDTH`** / **`GEN_CAP_STACKED_ROW_HEIGHT`**: one **row per** `n` (max `n` at top), **filled** histograms, shared x and **`HIST_BINS_DEFAULT`**; **`vertical`** viridis colorbar for `log10(n)` when **≥2** values of `n`. **`_distribution_panel`** (step + optional horizontal cbar) is kept for tests only. Sufficiency meta-analysis: constants **`META_*`**, **`SUFFICIENCY_*`** in `plot_config.py` (horizontal training-split bar under distribution grids; heatmap per-panel 0–1 norm + **“Max Structures”** x-label).
- **Dashboard (detail view):** Block sort order in `dashboard.py` matches **`analysis`** YAML: RMSD/Q → **coord + distmap clustering** → **generative capacity** → **latent**; **`meta_analysis_sufficiency`** is last among ordered types (scores still sort first). **Meta-Analysis** view (toolbar): full-width sufficiency heatmap + distribution stack per seed from manifest **`sufficiency_meta`**. Sandbox: **`dashboard_sandbox/`** — static preview via **`build_meta_analysis_page.py`** (see **`dashboard_sandbox/README.md`**).
