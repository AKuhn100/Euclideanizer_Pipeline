...
- **Package layout**: Core logic lives under `src/`. `run.py` imports from `src` (config, utils, metrics, plotting, train_*, analysis_metrics, rmsd, q_analysis, dashboard, gro_io). `run_setup_wizard.py` imports `src.wizard` and calls `main()`; wizard uses `src.wizard_prompts` for prompt construction. `run_hpo.py` imports `run` and uses `run.run_one_hpo_trial()` and run's logging. Tests live in `tests/` and use the same helpers as the main loop.
