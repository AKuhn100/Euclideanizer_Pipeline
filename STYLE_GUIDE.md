...
- **Single entrypoint**: `run.py`. All training, plotting, analysis, and dashboard are driven from here. There is no separate “plot-only” or “analysis-only” script; behavior is controlled by config and resume/overwrite logic.
- **Config is required**: No default config file. The pipeline is always invoked with `--config path/to/config.yaml`. All hyperparameters and options come from the config (with optional CLI overrides). Required keys are enforced in `src/config.py`; missing keys fail at load time.
- **Package layout**: Core logic lives under `src/`. `run.py` imports from `src` (config, utils, metrics, plotting, train_*, analysis_metrics, rmsd, q_analysis, dashboard, gro_io). Tests live in `tests/` and use the same helpers as the main loop.

### Running and testing

- Run: `python run.py --config samples/config_sample.yaml [--data path/to/data.gro]`
...
- **Lists in config:** Any distmap/euclideanizer key except `batch_size` can be a single value or a list. Lists are expanded into a Cartesian product (one run per combination). **`batch_size` must be a single integer** (no list). For `epochs`, a list means multi-segment training (e.g. [100, 300]: train to 100, then resume from that segment’s last and train to 300).
...
- **Paths:** Use `os.path.join()` for portability. Helpers like `_analysis_path()`, `_plot_path()` centralize output paths so that renames (e.g. min_rmsd → rmsd) happen in one place.
- **Logging:** Pipeline uses a custom log (e.g. `pipeline.log` in output root) plus mirror to stdout. `_log(..., style="info"|"skip"|"error")` for consistent formatting and elapsed time.

### 4.2 Config and tests

- **Sample config:** `samples/config_sample.yaml` must contain all required keys and serve as the reference. Test configs (`tests/config_test.yaml`, `tests/config_smoke.yaml`) use the same schema with minimal values.
