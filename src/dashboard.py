...
def _sufficiency_meta_sources_for_seed(seed_num: int, base_output_dir: str, seed_dir: str) -> dict[str, Any] | None:
    """Relative paths from seed_dir to sufficiency figures (for asset copy + Meta-Analysis view)."""
    suff_seed = os.path.join(
        base_output_dir,
        "meta_analysis",
        "sufficiency",
        f"seed_{seed_num}",
    )
    heat = os.path.join(suff_seed, "heatmap", "sufficiency_heatmap_rmsd_q.png")
    dist_root = os.path.join(suff_seed, "distributions")
    out: dict[str, Any] = {}
    if os.path.isfile(heat):
        out["heatmap_source"] = os.path.relpath(heat, seed_dir)
    ...
    if os.path.isdir(dist_root):
        ...
        for sub in sorted(os.listdir(dist_root), key=_natural_key):
            if not sub.startswith("max_data_"):
                continue
            png = os.path.join(dist_root, sub, "distributions_rmsd_q.png")
            if os.path.isfile(png):
                tag = sub[len("max_data_") :]
...
def _copy_assets_and_update_paths(
    ...
    for run in runs:
        ...
        if run.get("level") == "seed" and run.get("sufficiency_meta_sources"):
            sm_src = run["sufficiency_meta_sources"]
            root = run.get("run_root") or ""
            sid_safe = re.sub(r"[^\w.-]", "_", run["id"])
            smeta_out: dict[str, Any] = {}
            hs = sm_src.get("heatmap_source")
            if hs and root:
                hsrc = os.path.join(root, hs)
                if os.path.isfile(hsrc):
                    hname = f"{sid_safe}_sufficiency_heatmap.png"
                    if hname not in copied:
                        shutil.copy2(hsrc, os.path.join(assets_dir, hname))
                        copied.add(hname)
                    smeta_out["heatmap"] = f"assets/{hname}"
            dists_out: list[dict[str, str]] = []
            for d in sm_src.get("distributions") or []:
                if not root:
                    break
                dsrc = os.path.join(root, d.get("source", ""))
                if not os.path.isfile(dsrc):
                    continue
                tag = d.get("tag", "x")
                tag_safe = re.sub(r"[^\w.-]", "_", str(tag))
                dname = f"{sid_safe}_sufficiency_dist__{tag_safe}.png"
                if dname not in copied:
                    shutil.copy2(dsrc, os.path.join(assets_dir, dname))
                    copied.add(dname)
...
<head>
  ...
  <style>
    ...
    .meta-analysis-wrap { margin: 0 1.5rem 2.5rem; }
    .meta-page-nav { position: sticky; top: 0; z-index: 5; background: var(--bg-toolbar); border: 1px solid var(--border); border-radius: 8px; padding: 0.65rem 1rem; margin-bottom: 1.25rem; display: flex; flex-wrap: wrap; align-items: center; gap: 0.5rem 1rem; }
    ...
    .meta-seed-section { scroll-margin-top: 4.5rem; }
    .meta-subheading { font-size: 1rem; color: var(--text-secondary); margin: 1.25rem 0 0.75rem 0; font-weight: 600; }
    .meta-dist-grid { display: grid; grid-template-columns: 1fr; gap: 1rem; width: 100%; }
...
<body>
  ...
  <script>
    ...
    function blockTypeOrder(type, name) {
      ...
      // Align with analysis order in config YAML: RMSD/Q → clusterings → generative capacity → latent → (seed-level) sufficiency meta.
      const order = [
    ...
    function renderMetaAnalysisPage() {
      ...
      const navItems = seeds.map(s => {
        const safeId = 'meta-seed-' + String(s.id).replace(/[^a-zA-Z0-9_-]/g, '_');
        return '<li><a href="#' + escapeHtml(safeId) + '">' + escapeHtml(s.label_short || s.id) + '</a></li>';
      }).join('');
      let body = '<div class="meta-analysis-wrap">';
      body += '<nav class="meta-page-nav" aria-label="Seed Sections"><span class="meta-page-nav-label">Jump To</span><ul>' + navItems + '</ul></nav>';
      seeds.forEach(s => {
        const sm = s.sufficiency_meta;
        const safeId = 'meta-seed-' + String(s.id).replace(/[^a-zA-Z0-9_-]/g, '_');
        body += '<section class="run-card meta-seed-section" id="' + escapeHtml(safeId) + '">';
        body += '<h2 class="run-card-title">' + escapeHtml(s.label_short || s.id) + '</h2>';
        if (sm.heatmap) {
        ...
        if (dists.length) {
          body += '<h3 class="meta-subheading">Distributions By Max Data</h3><div class="meta-dist-grid">';
          dists.forEach(d => {
            body += '<div class="meta-dist-card"><div class="meta-dist-card-title">Max Data = ' + escapeHtml(String(d.max_data)) + '</div>';
            body += '<img src="' + escapeHtml(d.path) + '" alt="" loading="lazy"></div>';
          });
          body += '</div>';
        }
        body += '</section>';
    ...
    function updateContent() {
      ...
      if (state.viewMode === 'meta_analysis') {
        breadcrumbEl.style.display = 'none';
        renderMetaAnalysisPage();
        return;
      }
